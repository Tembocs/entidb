...
## 7) FFI Memory Safety Violation in `entidb_ffi`

**Finding**

- `EntiDbBuffer::from_vec` converts a `Vec` into a `Box<[u8]>` (shrinking capacity) and leaks it via `mem::forget`.
- `entidb_free_buffer` attempts to free this memory by reconstructing a `Vec` using `Vec::from_raw_parts`.
- This is **Undefined Behavior** because `Box` allocation layout is not guaranteed to match `Vec`'s expectation (especially regarding capacity).

**Why it matters**

- This is a severe memory safety violation that can cause heap corruption, crashes, or security vulnerabilities.
- It violates the "Safety" invariant in `docs/invariants.md`.

**Best solution**

- Change `EntiDbBuffer::from_vec` to use `Vec::into_raw_parts` (or `ManuallyDrop`) to preserve the original capacity and allocation layout.
- Ensure `entidb_free_buffer` receives exactly the pointer, length, and capacity that `Vec::from_raw_parts` expects.
...
- [x] FFI buffer memory safety fixed (Vec allocation preserved correctly).
...
### Fix 7: FFI buffer memory safety

**File:** `crates/entidb_ffi/src/buffer.rs`

Changed `EntiDbBuffer::from_vec` to use `ManuallyDrop` instead of `into_boxed_slice()`. The old code
converted the Vec to a Box<[u8]> (shrinking capacity to length) then leaked it, but `entidb_free_buffer`
expected Vec allocation semantics. This was undefined behavior. The fix preserves the original Vec's
pointer, length, and capacity so `Vec::from_raw_parts` in `entidb_free_buffer` receives exactly what it expects.
